    ...
    if($_SERVER['REQUEST_METHOD']=='POST')
            {
                ...
                if($numExistRows<1)
                    ...
                    $sql= "DELETE FROM `complaint` WHERE `Username`='$Username'";
                      $result=mysqli_query($conn,$sql);
                      if($result){
                        echo "<script>alert('Success! Your Complaint Deleted Successfully!');window.location='/gkvhms/student/suggestioncomplaint.php'</script>";
                        }
                       else{
                        echo "<script>alert('Your Complaint Not Deletd Due To Technical Issue Please Try Again ! ');window.location='/gkvhms/student/suggestioncomplaint.php'</script>";
                          }
                     }
             }
?>